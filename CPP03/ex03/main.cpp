...
void depleteEnergy(ClapTrap &trap, const std::string &target,
                   const std::string &trapType) {
  unsigned int count = 0;
  std::cout << "\nDepleting " << trapType << "'s energy..." << std::endl;

  // Keep attacking until energy is depleted
  for (int i = 0; i < 15; i++) { // Max 15 attacks to avoid infinite loop
...
int main() {
  ...
  polyTrap->attack("Target"); // Should use ClapTrap's attack since FragTrap
                              // doesn't override it
